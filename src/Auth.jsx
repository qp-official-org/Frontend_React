...
useEffect(() => {
    ...
    .then((res) => {
        ...
        setCode(res.data);
        // 회원가입 API 호출
        return axios.post("/users/sign_up", { headers: { "Authorization": `Bearer ${res.data.access_token}` }});
    })
    .then((res) => {
        // 회원가입 응답을 받아 사용자의 로그인 상태 확인
        if (res.data.isSuccess) {
            // 처음 로그인하는 사용자라면 닉네임 설정 페이지로 이동
            navigate('/Nickname');
        } else {
            // 이미 로그인한 적이 있는 사용자라면 메인 페이지로 이동
            navigate('/Main');
        }
    })
    .catch((err) => {
